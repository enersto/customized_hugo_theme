...
glmControl <- trainControl(method='cv', number=3, returnResamp='none', 
                           summaryFunction = twoClassSummary, classProbs = TRUE)

m_glm <- train(y ~ ., data=train_woe,method="glm",family="binomial",
             trControl=glmControl, metric = "ROC")
summary(m_glm)
...
m_gbm <- train(y ~ ., data=train_woe,method="gbm",
             trControl=rfControl,metric = "ROC")
...
predi_glm <- predict(m_glm,test_woe,type = "prob")

predi_gbm <- predict(m_gbm,test_woe,type = "prob")


twoClassSummary(test_woe, lev = levels(test_woe$y))
